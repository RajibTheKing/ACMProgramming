...
#define SIZE_N 100000      //#define is used for constant & global
...
int main()
{
    ...
    ll i,j,test,Case=1,total,N,minv,cnt,f;
    total=seive();
    ...
    while(test--)
    {
        ...
        for(i=0;prime[i]<N && i<total;i++)
        {
            if(N%prime[i]==0)
            {
                ...
                if(minv==INF) minv=cnt;
                else minv=gcd(minv,cnt);
            }
        }
        if(N>1)
            minv=1;
