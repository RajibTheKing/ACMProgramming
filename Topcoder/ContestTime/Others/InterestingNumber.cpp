...
using namespace std;
#define F "Not interesting"
#define T "Interesting"
...
public:
	string isInteresting(string x)
	{
        ...
        for(int i=0;i<10;i++)
        {
            if(adj[i].size()%2==1) return F;
            if(adj[i].size()>2) return F;
            for(int j=0;j<adj[i].size();j++)
            {
                if(j==adj[i].size()-1) break;
                if(i!=(adj[i][j+1] - adj[i][j] - 1) ) return F;
            }
        }
        return T;
	}
